...
# Функція що виводить студентів по веденій групі
def grup(d):
    grupp = input("Введіть групу: ")
    found = False
    for name, elements in d.items():
        if grupp == elements["група"]:
            found = True
            print(f"{name}: {elements}")
    if not found:
        print(f"Немає студентів у групі '{grupp}'")

# Функція, що видаляє студента зі списку
def d_delete(d):
    name = input("Введіть ім'я студента для видалення у форматі 'ім'я прізвище по батькові': ")
    if name in d:
        confirm = input(f"Ви дійсно хочете видалити {name}? Так / Ні: ").lower()
        if confirm == "так":
            del d[name]
...
while True:
    ...
    c = input("Введіть команду: ").strip().lower()
    # Обробка команди
    if c == "команди":  # Вивід списку всіх команд
        print("'вивід' - вивести інформацію.",
              "\n'додати' - додати нового студента або змінити дані вже існуючого",
              "\n'група' - виведення студентів по групі.",
              "\n'видалити' - видалити студента зі списку.",
